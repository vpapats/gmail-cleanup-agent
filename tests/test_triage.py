...
class _State:
    def __init__(self, message_ids=None, *, fail_load=False, fail_save=False):
    ...
    def save(self, message_ids):
        self.calls.append(("save", tuple(message_ids)))
        if self.fail_save:
...
def test_wrongly_trashed_feedback_is_reviewed_in_summary_then_kept():
    ...
    assert runner.feedback_state.calls == [("save", ("m1",))]
    assert runner.gmail.calls.index(send_call) < runner.gmail.calls.index(
        ("remove", "m1", "feedback-id")
    )
...
def test_feedback_memory_load_failure_stops_before_labels_can_be_finalized():
    runner = _runner(_context(labels=["INBOX"]), daily_summary_enabled=True)
    runner.feedback_state = _State(fail_load=True)
...
def test_feedback_is_not_marked_kept_when_daily_summary_send_fails():
    ...
    assert runner.feedback_state.calls == []


def test_feedback_labels_remain_pending_when_memory_save_fails():
    runner = _runner(_context(labels=["INBOX"]), daily_summary_enabled=True)
    runner.gmail.message_exists = True
    runner.feedback_state = _State(fail_save=True)
    reviews, _, _, _ = runner._process_feedback([])
